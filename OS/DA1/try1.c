  ...
  printf("\n");
    ...
    for(ind; data[ind][0] - ch == 0; ind++){
           ...
           if(j == length){
            ...
            //printf("%lld is value of num", num);
           }
            }
     }
...
void read_data(){
  ...
  length = 0;
  c = 0;
  filePointer = fopen("final_test.txt", "r");
  if(filePointer == NULL){
    perror("File not opened");
  }
  while(!feof(filePointer) && (o < 1000)){
       c = getc(filePointer);
      if(c == '\n'){
      data[length++][j] = '\0';
       o++;
...
void process_data(){
    ...
    for(index_global; index_global <= o;index_global++){
        ...
        case 'z':
            ...
            break;
        default:
           // printf("Cannot identify the character\n");
            break;
        }
