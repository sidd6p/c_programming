...
pthread_mutex_t mutex1;//so that thread1 execute before thread2
...
int main(void){
  ...
  //both thread execute concorrenlty, but thread2 will get block by mutex1 and start after the lock release by thread1.
...
void * shm_writer(void * arg){
    ...
    void *addr_writer;
    char msg_writer[MESSAGE_LENGTH];
    int c = 0, i = 0;
    ...
    while((c = msg_writer[i]) != '\0'){
        if((c >= 65 && c <= 90)){//for ecoding A-Z to F-E
        c = ((c / 86)*(c - 26) + (85 / c)*(c)) + 5;
        }
        msg_writer[i++] = c;
    }
    msg_writer[i] = '\0';
...
void * shm_reader(void * arg){
    ...
    while((c = (msg_reader[i])) != '\0'){
        if((c >= 65 && c <= 90)){//for decoding F-E to A-Z
        c = ((69 / c)*(c + 26) + (c / 70)*(c)) - 5;
        }
        msg_reader[i++] = c;
    }
    msg_reader[i] = '\0';
