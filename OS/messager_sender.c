...
/* For mode constants */
#include <mqueue.h>//message queues (REALTIME)
//this also contain <fcntl.h> 
...
int main(){
    ...
    char msg[128];
    char *queueName = "/test"; 
    mqDesc = mq_open(queueName, O_RDWR|O_CREAT, 0660, NULL);// mqd_t mq_open(const char *name, int oflag, mode_t mode, struct mq_attr *attr);
    ...
    if(mq_send(mqDesc, msg, strlen(msg), 0) == -1){//int mq_send(mqd_t mqdes, const char *msg_ptr, size_t msg_len, unsigned int msg_prio);
